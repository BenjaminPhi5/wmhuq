...
def calc_img_features(synthseg_out, vent_dist_map, img, qc=False):
    thresh_feats = {}
    thresholds = [0.2]
    if qc:
...
def extract_features(synthseg_out, vent_dist, samples, mean_pred, umap, qc=False):
    sample_feats = {f"sample_{i}":calc_img_features(synthseg_out, vent_dist, s, qc) for (i, s) in enumerate(samples)}
    mean_feats = {f"mean":calc_img_features(synthseg_out, vent_dist, mean_pred, qc)}
    umap_feats_raw = {"umap_raw":calc_img_features(synthseg_out, vent_dist, umap, qc)}
    # umap2 = umap.clone()
    ...
    if qc:
        ...
        umap_feats_atseg = {"umap_atseg": calc_img_features(synthseg_out, vent_dist, umap3, qc)}
