...
class HausdorffDTLoss(nn.Module):
    ...
    def forward(
        ...
        dt_field = pred_error * distance.to(pred_error.device)
        loss = dt_field.mean()
...
### the various torch deterministic losses
...
def mean_sum_xent_loss(weight):
    def lfunc(pred, target):
        l = torch.nn.functional.cross_entropy(pred, target, weight=weight.to(pred.device), reduction='none')
        bs = l.shape[0]
        l = l.view(bs, -1)
...
def mean_AVD(pred, target):
    pred = torch.nn.functional.softmax(pred, dim=1)[:,1]
    segmented = pred > 0.5
    pred_segmented = pred * segmented #  keep just the segmented voxels
    bs = pred.shape[0]
    
    avd = (pred_segmented - target).abs()
    avd = avd.view(bs, -1)
    avd = avd.sum(dim=1)
    avd = avd.mean()
...
def mean_AVD_2(pred, target):
    pred = torch.nn.functional.softmax(pred, dim=1)[:,1]
    segmented = pred > 0.5
    pred_segmented = pred * segmented #  keep just the segmented voxels
    bs = pred.shape[0]
    
    pred_segmented = pred_segmented.view(bs, -1)
    target = target.view(bs, -1)
    avd = (pred_segmented.sum(dim=1) - target.sum(dim=1)).abs()
    return avd.mean()
...
def main(args):
    ...
    losses = {
        ...
        "avd":mean_AVD_2,
        "hausdorff":HausdorffDTLoss(alpha=2),
        "xent":xent_loss(weight, args.reduction),
        "tversky":TverskyLoss(args.beta)
    }
    loss = losses[loss_name]

    optimizer_params={"lr":1e-4, "weight_decay":args.weight_decay}
