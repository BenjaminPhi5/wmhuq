...
class PUNetLitModelWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat_prior = self.model.sample(testing=False, mean=True) # or I can use reconstruct to get samples from the posterior...
        #y_hat = self.model.reconstruct(use_posterior_mean=False, calculate_posterior=True)
        y_hat_posterior = self.model.reconstruct(use_posterior_mean=True, calculate_posterior=False)
        loss = self.loss(self.model, y_hat_prior, y_hat_posterior, y)
        
        # metrics 
        if self.logging_metric_train:
            self.logging_metric_train(y_hat_posterior, y)
...
#         self.log('val_metric_epoch', self.logging_metric_val.compute())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #y_hat = self.model.reconstruct(use_posterior_mean=False, calculate_posterior=True)
        y_hat_posterior = self.model.reconstruct(use_posterior_mean=True, calculate_posterior=False)
        val_loss = self.loss(self.model, y_hat_prior, y_hat_posterior, y)
        # val_loss = self.loss(self.model, y_hat, y)
        
        if self.logging_metric_val:
            self.logging_metric_val(y_hat_prior, y)
...
def main(args):
    ...
    def punet_loss(net, pred1, pred2, target):
        ...
        dice = (dice_loss(pred1, target) + dice_loss(pred2, target))/2
