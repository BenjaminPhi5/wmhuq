...
def main(args):
    ...
    def gen_samples(xs3dQ, ys3dQ):
        ...
        folder = "Xnew_mcdrop_01/"
        if variant == "01":
            c = 'epoch=28-step=4263.ckpt'
        elif variant== "02":
            c = 'epoch=32-step=4851.ckpt'
        else:
            c = 'epoch=30-step=4557.ckpt'
        # c=

        model = StandardLitModelWrapper.load_from_checkpoint(root + folder + c, model=model_raw, loss=loss, 
        ...
        for i in tqdm(range(len(ys3dQ)), position=0, leave=True, ncols=150):
            ...
            with torch.no_grad():
                ...
                for j in range(20):
                    ...
                    samples3d[i].append(out/t)
                model.eval()
                out = model(xs3dQ[i].swapaxes(0,1).cuda()).cpu()
                means3d.append(out/t)
