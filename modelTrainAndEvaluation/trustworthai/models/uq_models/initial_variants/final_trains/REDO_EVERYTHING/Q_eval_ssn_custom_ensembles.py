...
def main(args):
    ...
    # define the model.
    model_raw = model_raw = HyperMapp3rSSN2(ssn_rank=15).cuda()
    ...
    def gen_samples(xs3dQ, ys3dQ):
        ...
        if variant == "best_small":
            folders = [root2 + "Xnew_ssn_ens" + ndigit(2, x) + "/" for x in [3,5,9]] + [root2 + "new_ssn_ens" + ndigit(2, x) + "/" for x in [12, 19]]
        
        elif variant == "best_big":
            folders = (
                [root2 + "Xnew_ssn_ens" + ndigit(2, x) + "/" for x in [3,5,9]]
                + [root2 + "new_ssn_ens" + ndigit(2, x) + "/" for x in [12, 19]]
                + [
                    root1 + fi + "/" for fi in ["REDO_ssn06_r10_dl5_sdf0.5",
        ...
        elif variant == "mu_only":
            folders = [root2 + "Xnew_ssn_ens" + ndigit(2, x+1) + "/" for x in range(10)]
        else:
            folders = [root2 + "new_ssn_ens" + ndigit(2, x+11) + "/" for x in range(10)]
        ...
        for i in range(len(folders)):
            ...
            for j in tqdm(range(len(xs3dQ)), position=0, leave=True, ncols=150):
                ...
                with torch.no_grad():
                    ...
                    t = 1.5
                    if variant == "best_small":
                        samples = fixed_re_parametrization_trick(out['distribution'], 4).cpu()
                    else:
                        samples = fixed_re_parametrization_trick(out['distribution'], 2).cpu()
                    for s in samples:
                        samples3d[j].append(s/t)
                    mean = out['distribution'].mean.cpu()
                    model_means3d[j].append(mean/t)

        model_means3d = [torch.stack(mus) for mus in model_means3d]
        means3d = [torch.mean(m, dim=0) for m in model_means3d]
