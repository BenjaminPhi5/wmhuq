...
def construct_parser():
    ...
    parser.add_argument('--sample_dice_factor', default=None, type=float)
...
class SsnNetworkSampleLossWrapper(nn.Module):
    def __init__(self, loss_func, dice_factor, sample_dice_factor, samples=10):
        super().__init__()
        self.loss = loss_func
        self.samples = samples
        self.dice_factor = dice_factor
        self.sample_dice_factor = sample_dice_factor
    def forward(self, result_dict, target):
        samples = fixed_re_parametrization_trick(result_dict['distribution'], self.samples).to(target.device)
        loss = 0
        mean = result_dict['logit_mean']
        mean_loss = self.loss(mean, target) * self.dice_factor
        for s in samples:
            loss += self.loss(s, target)
        return mean_loss + ((loss * self.sample_dice_factor) / self.samples)
...
def main(args):
    ...
    rank = args.rank
    sample_dice_factor = args.sample_dice_factor
    
    
    ssn_diceloss = SsnNetworkSampleLossWrapper(dice_loss, dice_factor, sample_dice_factor)
    mc_loss = StochasticSegmentationNetworkLossMCIntegral(num_mc_samples=10)
    
    def double_loss(outs, target):
        return ssn_diceloss(outs, target) + mc_loss(outs, target) * 0.01
    
    ckpt_dir = args.ckpt_dir + f"_r{rank}_dl{dice_factor}_sdf{sample_dice_factor}"
    is3D = False
    ...
    # define the model.
    model_raw = model_raw = HyperMapp3rSSN2(ssn_rank=rank).cuda()
