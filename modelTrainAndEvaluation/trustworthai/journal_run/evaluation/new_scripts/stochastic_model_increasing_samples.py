...
def main(args):
    ...
    for num_samples in [2, 3, 5, 7, 10, 15, 20, 25, 30]:
        ...
        try:
            ...
            means, samples, misc = get_means_and_samples(model_raw, eval_ds, num_samples=num_samples, model_func=MODEL_OUTPUT_GENERATORS[args.uncertainty_type], args=args)
            ...
            if samples[0] is not None:
                ...
                # best dice, avd, rmse
                chal_results[f'best_dice_ss{num_samples}'] = sample_top_dices
                chal_results[f'best_avd_ss{num_samples}'] = sample_best_avds
                ...
                chal_results[f'best_dice_dsorted_ss{num_samples}'] = best_dice

                # ged score
                geds = iou_GED(means, ys3d_test, samples)
                chal_results[f'GED^2_ss{num_samples}'] = geds
            else:
                break
