...
from trustworthai.journal_run.new_MIA_fazekas_and_QC.generating_model_samples_outputs.utils import *
...
def main(args):
    ...
    domains_chal = ["test_Amsterdam_GE1T5", "test_Amsterdam_Philips_VU_PETMR_01", "test_Utrecht", "test_Amsterdam_GE3T", "test_Singapore"]
    
    chal_ds = ConcatDataset([
    ...
    IDs_all = [out[2] for out in chal_ds]
    ...
    while end < 300:
        ds = LimitedSizeDataset(chal_ds, start=start, end=end)
        ...
        # write results
        write_results_to_disk(args, IDs, samples, output_dir=args.result_dir)
