...
- [x] no more thresholding, we pick a minimum base threshold (e.g 0.2) and look at everything above that threshold
  - [x] with sums computed as soft values.
  - [x] connected component identification is done by then binarizing at everything greater than 0, but again the size is taken as a soft sum!
- [x] the caudate feature on its own will not be stored
- [x] the uncertainty maps will be unedited, i.e they will not be combined with the prediction map. (the filled in version will be retained as a separate feature)
- [x] the addition of the sum of the uncertainty / the sum of the segmentation (spatial separation of the uncertainty!) instead, we are just doing the uncertainty where the segmentation is.

- later on at the analysis stage, we will remove the computation of the p75 and p25 regions per cc.
...
def get_features(all_samples, model_outs, IDs, vent_dists, synthseg_outs):
    ...
    for pid in tqdm(IDs, position=0, leave=True, ncols=100):
        ...
        pid_feats[pid].update(umap_feats_filled)
        pid_feats[pid].update(umap_feats_atseg)
...
def main(args):
    ...
    ### save the df
    df.to_csv(os.path.join(out_folder_name, f"{model_name}_per_sample_new_features_v3.csv"))
