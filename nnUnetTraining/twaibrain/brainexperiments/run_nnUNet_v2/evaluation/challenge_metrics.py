...
import numpy as np
import torch
from twaibrain.braintorch.utils.print_and_write_func import print_and_write
from twaibrain.braintorch.utils.logits_to_preds import mle_batch
...
def dice_across_samples_analysis(results_text_file, ys3d, samples3d):
    ...
    # [s][ind] not [ind][s]
    dices3d = []
    for ind in tqdm(range(len(samples3d)), position=0, leave=True, ncols=100):
        sample_dices = []
